...
enum CheckpointAppGroup {
    static let identifier = "group.Capybarista.CheckPoint"
}
...
enum CheckpointDeepLink {
    ...
    static func gameID(from url: URL) -> UUID? {
        guard url.scheme?.lowercased() == scheme,
              url.host?.lowercased() == "game" else {
            return nil
        }

        let rawID = url.path.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
...
enum CheckpointWidgetSnapshotStore {
    ...
    static func load() -> CheckpointWidgetSnapshot {
        guard let defaults = UserDefaults(suiteName: CheckpointAppGroup.identifier),
    ...
    static func save(_ snapshot: CheckpointWidgetSnapshot) {
        guard let defaults = UserDefaults(suiteName: CheckpointAppGroup.identifier) else {
            return
        }
